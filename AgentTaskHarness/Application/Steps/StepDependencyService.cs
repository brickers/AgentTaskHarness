...
public class StepDependencyService(AppDbContext dbContext)
{
	...
	{
		if (stepId == dependsOnStepId) throw new InvalidOperationException("A step cannot depend on itself.");
		...
		if (await dbContext.StepDependencies.AnyAsync(d => d.StepId == stepId && d.DependsOnStepId == dependsOnStepId,
	...
	{
		return dbContext.StepDependencies
			.AsNoTracking()
	...
	{
		if (step.WorkflowColumn == WorkflowColumn.Backlog) return;

		if (step.WorkflowColumn == WorkflowColumn.Ready && step.BranchName is null) return;

		throw new InvalidOperationException(
			"Step dependencies can only be changed before starting or after returning to the backlog.");
	}

	private async Task<bool> WouldCreateCycleAsync(Guid stepId, Guid dependsOnStepId,

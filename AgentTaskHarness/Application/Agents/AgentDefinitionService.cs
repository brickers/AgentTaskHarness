...
public class AgentDefinitionService(AppDbContext dbContext, IAgentDefinitionFolderWriter folderWriter)
{
	...
	public Task<AgentDefinition?> GetByIdAsync(Guid definitionId, CancellationToken cancellationToken = default) =>
		dbContext.AgentDefinitions.AsNoTracking()
	...
	public async Task<AgentDefinition> SaveAsync(Guid definitionId, string name, string folderPath, IReadOnlyList<Guid> componentIds, CancellationToken cancellationToken = default)
	{
		Validate(name, folderPath);
	...
	public Task<List<AgentComponent>> GetComponentsAsync(CancellationToken cancellationToken = default) =>
		dbContext.AgentComponents.AsNoTracking().OrderBy(component => component.Name).ToListAsync(cancellationToken);

	public async Task<AgentComponent> CreateComponentAsync(string name, string configContent, CancellationToken cancellationToken = default)
	{
	...
	public async Task<AgentComponent> UpdateComponentAsync(Guid componentId, string name, string configContent, CancellationToken cancellationToken = default)
	{
		ValidateComponent(name, configContent);
	...
	{
		var component = await FindComponentAsync(componentId, cancellationToken);
		if (await dbContext.AgentDefinitionComponents.AnyAsync(reference => reference.ComponentId == componentId, cancellationToken))
		{
	...
	public async Task AddComponentAsync(Guid definitionId, Guid componentId, CancellationToken cancellationToken = default)
	{
		var definition = await FindDefinitionWithComponentsAsync(definitionId, cancellationToken);
		var component = await FindComponentAsync(componentId, cancellationToken);
		if (definition.Components.Any(reference => reference.ComponentId == componentId))
		{
			return;
		}
	...
	public async Task RemoveComponentAsync(Guid definitionId, Guid componentId, CancellationToken cancellationToken = default)
	{
		var definition = await FindDefinitionWithComponentsAsync(definitionId, cancellationToken);
		var reference = definition.Components.SingleOrDefault(reference => reference.ComponentId == componentId)
			?? throw new KeyNotFoundException($"Component '{componentId}' is not part of agent definition '{definitionId}'.");
		dbContext.AgentDefinitionComponents.Remove(reference);
		definition.Components.Remove(reference);
		await NormalizeComponentOrderAsync(definition.Components.OrderBy(component => component.Order).ToList(), cancellationToken);
	}

	public async Task ReorderComponentsAsync(Guid definitionId, IReadOnlyList<Guid> componentIds, CancellationToken cancellationToken = default)
	{
		ArgumentNullException.ThrowIfNull(componentIds);
		var definition = await FindDefinitionWithComponentsAsync(definitionId, cancellationToken);
		if (definition.Components.Count != componentIds.Count || componentIds.Distinct().Count() != componentIds.Count || definition.Components.Select(component => component.ComponentId).Except(componentIds).Any())
		{
			throw new InvalidOperationException("The component order must exactly match the agent definition's components.");
		}

		var references = definition.Components.ToDictionary(component => component.ComponentId);
		await NormalizeComponentOrderAsync(componentIds.Select(componentId => references[componentId]).ToList(), cancellationToken);
	}
	...
	private async Task<AgentDefinition> FindDefinitionWithComponentsAsync(Guid definitionId, CancellationToken cancellationToken) =>
		await dbContext.AgentDefinitions
	...
	private async Task<AgentComponent> FindComponentAsync(Guid componentId, CancellationToken cancellationToken) =>
		await dbContext.AgentComponents.SingleOrDefaultAsync(component => component.Id == componentId, cancellationToken)
		?? throw new KeyNotFoundException($"Agent component '{componentId}' was not found.");

	private async Task ReplaceComponentsAsync(AgentDefinition definition, IReadOnlyList<Guid> componentIds, CancellationToken cancellationToken)
	{
		if (componentIds.Distinct().Count() != componentIds.Count)
		{
			throw new InvalidOperationException("An agent definition cannot contain the same component more than once.");
		}

		var components = await dbContext.AgentComponents.Where(component => componentIds.Contains(component.Id)).ToDictionaryAsync(component => component.Id, cancellationToken);
		...
		foreach (var reference in definition.Components.Where(reference => !componentIds.Contains(reference.ComponentId)).ToList())
		{
			dbContext.AgentDefinitionComponents.Remove(reference);
			definition.Components.Remove(reference);
		}
	...
	private async Task NormalizeComponentOrderAsync(IReadOnlyList<AgentDefinitionComponent> orderedComponents, CancellationToken cancellationToken)
	{
		for (var index = 0; index < orderedComponents.Count; index++)
		{
			orderedComponents[index].Order = -index - 1;
		}
		await dbContext.SaveChangesAsync(cancellationToken);

		for (var index = 0; index < orderedComponents.Count; index++)
		{
			orderedComponents[index].Order = index;
		}
		await dbContext.SaveChangesAsync(cancellationToken);
	}

	...
	StepDependencyService? stepDependencyService = null)
{
	...
	public Task<Feature> MoveAsync(Guid featureId, WorkflowColumn targetColumn,
		CancellationToken cancellationToken = default)
	{
		return MoveAsync(featureId, targetColumn, false, false, cancellationToken);
	}

	public Task<Feature> MoveAndDiscardWorktreeAsync(Guid featureId, WorkflowColumn targetColumn,
		CancellationToken cancellationToken = default)
	{
		return MoveAsync(featureId, targetColumn, true, false, cancellationToken);
	}

	public Task<Feature> MoveAsync(Guid featureId, WorkflowColumn targetColumn, bool discardWorktreeOnBacklogReturn,
		CancellationToken cancellationToken = default)
	{
		return MoveAsync(featureId, targetColumn, discardWorktreeOnBacklogReturn, false, cancellationToken);
	}

	public Task<Feature> MoveMcpAsync(Guid featureId, WorkflowColumn targetColumn,
		CancellationToken cancellationToken = default)
	{
		return MoveAsync(featureId, targetColumn, false, true, cancellationToken);
	}

	public async Task<Feature> MoveAsync(Guid featureId, WorkflowColumn targetColumn,
		bool discardWorktreeOnBacklogReturn, bool isMcpMove, CancellationToken cancellationToken = default)
	{
		...
		var canSkipHumanReview = feature.Board is not null && feature.Board.SkipFeatureHumanReview &&
		...
		if (feature.WorkflowColumn == WorkflowColumn.AgentReview && targetColumn == WorkflowColumn.HumanReview &&
		...
		if (!isAllowed)
			throw new InvalidOperationException(
				$"Moving from '{feature.WorkflowColumn}' to '{targetColumn}' is not allowed.");
		...
		if (feature.WorkflowColumn == WorkflowColumn.Backlog && effectiveTarget == WorkflowColumn.Ready)
		{
			...
			if (!dependenciesMet)
				throw new InvalidOperationException(
					"All feature dependencies must be completed before moving to Ready.");
		}
		...
		try
		{
			if (effectiveTarget == WorkflowColumn.Backlog && discardWorktreeOnBacklogReturn)
				await _gitWorktrees.DiscardFeatureWorktreeAsync(feature, cancellationToken);
			else
				await _gitWorktrees.HandleFeatureTransitionAsync(feature, previousColumn, effectiveTarget,
					cancellationToken);
		}
		...
		if (previousColumn == WorkflowColumn.Backlog && effectiveTarget == WorkflowColumn.Ready)
		{
			...
			foreach (var step in backlogSteps) step.WorkflowColumn = WorkflowColumn.Ready;
		}
	...
	public async Task<Feature> ResumeMergeAsync(Guid featureId, CancellationToken cancellationToken = default)
	{
		...
		await _gitWorktrees.ResumeFeatureMergeAsync(feature, cancellationToken);
	...
	public async Task DiscardUncommittedChangesAsync(Guid featureId, CancellationToken cancellationToken = default)
	{
		...
		await _gitWorktrees.DiscardFeatureUncommittedChangesAsync(feature, cancellationToken);
	}
	...
	public async Task<IReadOnlyList<WorkflowColumn>> GetAllowedMovesAsync(Guid featureId,
		CancellationToken cancellationToken = default)
	{
		var feature = await dbContext.Features
			              .Include(f => f.Board)
		...
		if (feature.WorkflowColumn == WorkflowColumn.Done || feature.MergeConflictPending || hasActiveAgent) return [];
		...
		foreach (var target in candidates)
		{
			...
			if (feature.WorkflowColumn == WorkflowColumn.AgentReview && target == WorkflowColumn.HumanReview &&
			    canSkipHumanReview)
			{

...
public class WorkflowTransitionRules
{
	public bool IsAllowedTransition(WorkflowColumn current, WorkflowColumn target)
	{
		...
		if (current == WorkflowColumn.Done)
		{
			return false;
		}
		...
		if (target is WorkflowColumn.Backlog or WorkflowColumn.Ready)
		{
		...
		if (current is WorkflowColumn.AgentReview or WorkflowColumn.HumanReview && target == WorkflowColumn.Build)
		{
			return true;
		}
	...
	public IReadOnlyList<WorkflowColumn> GetAllowedTransitions(WorkflowColumn current)
	{
		...
		foreach (WorkflowColumn candidate in Enum.GetValues<WorkflowColumn>())
		{
	...
	public bool IsTerminal(WorkflowColumn column) => column == WorkflowColumn.Done;
}
